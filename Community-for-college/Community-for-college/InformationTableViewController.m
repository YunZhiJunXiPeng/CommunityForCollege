...
- (void)requestData
{
    NSString *newListURL = [NSString stringWithFormat:kInformationListURL,self.loadingCount*20];
    //请求数据
...
-(void)requestHandle:(RequstHandle *)requesthandle didSucceedWithData:(NSMutableData *)data
{
    ...
    NSArray *array = [dic valueForKey:kInformationKey];
    if (array == nil) {
        //提示讯息
...
//        [_cell.myTitle setFont:[UIFont systemFontOfSize:18] fromIndex:0 length:_cell.myTitle.text.length];
    ...
    if ([list valueForKey:@"imgextra"] != nil)
    {
        ...
        if (!cell1) {
            cell1 = [[CellForThreeImage alloc]initWithStyle:(UITableViewCellStyleValue2) reuseIdentifier:identifier1];
        }
    ...
    }else
    {
        ...
        if (!cell2) {
            cell2 = [[CellForOneImage alloc]initWithStyle:(UITableViewCellStyleValue2) reuseIdentifier:identifier2];
        }
