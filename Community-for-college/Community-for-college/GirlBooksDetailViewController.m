//
//  GirlBooksDetailViewController.m
...
@interface GirlBooksDetailViewController ()

@property (strong , nonatomic) GirlBookScrollView *ScrollView;

@property (strong , nonatomic) UIBarButtonItem *Collect;
@property (strong , nonatomic) UIBarButtonItem *share;
@end

@implementation GirlBooksDetailViewController

- (void)loadView
{
    self.navigationItem.title = @"图书详情";
    self.ScrollView = [[GirlBookScrollView alloc]initWithFrame:[UIScreen mainScreen].bounds];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (result == 0) {
        self.navigationItem.rightBarButtonItems[1].title = @"收藏";
    }else{
        self.navigationItem.rightBarButtonItems[1].title = @"取消";
    }
